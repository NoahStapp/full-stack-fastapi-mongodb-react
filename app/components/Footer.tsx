...
const mastodonIcon = () => {
    return (
        <svg fill="currentColor" viewBox="0 0 512 512" className="h-6 w-6" aria-hidden="true">
            <path fillRule="evenodd" 
...
const renderNavigation = () => {
    return footerNavigation.main.map(item => 
        <div key={item.name} className="px-5 py-2">
            <Link href={item.to} className="text-base text-gray-400 hover:text-gray-300">{item.name}</Link>
        </div>
    )
}
        ...
        <footer className="mt-24 bg-gray-900 sm:mt-12">
            <div className="mx-auto max-w-md overflow-hidden py-12 px-4 sm:max-w-3xl sm:px-6 lg:max-w-7xl lg:px-8">
                ...
                <div className="flex justify-between">
                    <p className="mt-8 text-center text-base text-gray-400">&copy; 2023 {siteName}. All rights reserved.</p>
                    {/* <LocaleDropdown /> */}
                </div>
            </div>
        </footer>
