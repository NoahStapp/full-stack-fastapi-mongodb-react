...
import { Cog8ToothIcon, UsersIcon, UserPlusIcon } from "@heroicons/react/24/outline"
    ...
    const renderNavigation = () => {
        return navigation.map(item => 
            <button 
                ...
                className={[item.id === selected ? 'text-rose-700 hover:text-rose-700' : 'text-gray-900 hover:text-gray-900', 'group rounded-md px-3 py-2 flex items-center text-sm font-medium'].join(' ')}
                onClick={() => changeSelection(item.id)}
            >
                <item.icon className={[item.id === selected ? 'text-rose-700 hover:text-rose-700' : 'text-gray-900 hover:text-gray-900', 'flex-shrink-0 -ml-1 mr-3 h-6 w-6'].join(' ')} aria-hidden="true" />
          ...
          <div className="lg:grid lg:grid-cols-12 lg:gap-x-5">
            <aside className="py-6 px-2 sm:px-6 lg:col-span-3 lg:py-0 lg:px-0">
              <nav className="space-y-1" aria-label="tabs">
                ...
                >
                  <Cog8ToothIcon className="text-gray-400 group-hover:text-gray-500 flex-shrink-0 -ml-1 mr-3 h-6 w-6" aria-hidden="true" />
